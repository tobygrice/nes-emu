...
class PPUCtrl {
  ...
  public:
    ...
    // Returns the VRAM address increment: 1 if the VRAM_ADD_INCREMENT flag is
    // not set, 32 otherwise.
    uint8_t vram_addr_increment() const {
        return !isSet(VRAM_ADD_INCREMENT) ? 1 : 32;
    }

    // Returns the sprite pattern table address: 0 if the flag is not set,
    // 0x1000 if it is.
    ...
    // Returns the sprite size: 8 if the SPRITE_SIZE flag is not set, 16 if it
    // is.
    ...
    uint8_t master_slave_select() const {
        return !isSet(MASTER_SLAVE_SELECT) ? 0 : 1;
    }

    // Returns true if an NMI should be generated at the start of vertical
    // blank.
